...
export GITHUB_REPOSITORY="alawein/qmatsim"   # optional; defaults to alawein/qmatsim
node scripts/github-sync-report.mjs
```

Artifact: `reports/sync-report.qmatsim.json` (gitignored). In **GitHub Actions**, run workflow **"Ops — GitHub sync report"** (workflow_dispatch); it uploads the JSON as a workflow artifact.
